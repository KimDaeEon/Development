...
void DBSynchronizer::ParseXmlDB(const WCHAR* path)
{
	...
	for (XmlNode& table : tables)
	{
		...
		myVector<XmlNode> columns = table.FindChildren(L"Column");
		for (XmlNode& column : columns)
		{
			DBModel::ColumnRef c = myMakeShared<DBModel::Column>();
		...
		myVector<XmlNode> indexes = table.FindChildren(L"Index");
		for (XmlNode& index : indexes)
		{
			DBModel::IndexRef i = myMakeShared<DBModel::Index>();
			...
			myVector<XmlNode> columns = index.FindChildren(L"Column");
			for (XmlNode& column : columns)
			{
	...
	{
		DBModel::ProcedureRef p = myMakeShared<DBModel::Procedure>();
		p->_name = procedure.GetStringAttr(L"name");
		p->_body = procedure.FindChild(L"Body").GetStringValue();

		myVector<XmlNode> params = procedure.FindChildren(L"Param");
...
bool DBSynchronizer::GatherDBTables()
{
	...
	while (getDBTables.Fetch())
	{
		...
		{
			table = myMakeShared<DBModel::Table>();
		...
		DBModel::ColumnRef column = myMakeShared<DBModel::Column>();
		{
...
bool DBSynchronizer::GatherDBIndexes()
{
	...
	while (getDBIndexes.Fetch())
	{
		...
		myVector<DBModel::IndexRef>& indexes = (*findTable)->_indexes;
		...
		{
			DBModel::IndexRef index = myMakeShared<DBModel::Index>();
			{
		...
		myVector<DBModel::ColumnRef>& columns = (*findTable)->_columns;
...
{
	WCHAR name[101] = { 0 };
	myVector<WCHAR> body(PROCEDURE_MAX_LEN);
	...
	{
		DBModel::ProcedureRef proc = myMakeShared<DBModel::Procedure>();
		{
			proc->_name = name;
			proc->_fullBody = myString(body.begin(), std::find(body.begin(), body.end(), 0));
		}
...
void DBSynchronizer::CompareDBModel()
{
	...
	for (myVector<myString>& queries : _updateQueries)
		queries.clear();

	// XML에 있는 목록을 우선 갖고 온다.
	myMap<myString, DBModel::TableRef> xmlTableMap;
...
void DBSynchronizer::ExecuteUpdateQueries()
{
	...
	{
		for (myString& query : _updateQueries[step])
		{
			_dbConn.UnBind();
...
void DBSynchronizer::CompareTables(DBModel::TableRef dbTable, DBModel::TableRef xmlTable)
{
	...
	myMap<myString, DBModel::ColumnRef> xmlColumnMap;
	for (DBModel::ColumnRef& xmlColumn : xmlTable->_columns)
	...
	myMap<myString, DBModel::IndexRef> xmlIndexMap;
	for (DBModel::IndexRef& xmlIndex : xmlTable->_indexes)
...
void DBSynchronizer::CompareStoredProcedures()
{
	...
	myMap<myString, DBModel::ProcedureRef> xmlProceduresMap;
