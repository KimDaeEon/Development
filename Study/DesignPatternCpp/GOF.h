...
namespace GOF
{
	...
	namespace Structural
	{
		...
		namespace Decorator
		{
			...
			컴포넌트가 무거우면 데코레이터 말고 전략패턴 쓰는 것이 더 낫다.
			주로 그래픽 처리할 때에 사용하거나 스트림 입출력 시에 선후처리를 하기 위해서 사용한다.
			구성이랑 비슷해 보이지만 사용 의도가 다르다.
			*/
		...
		namespace Proxy
		{
			...
			// 스마트 포인터도 일종의 프록시임
			// 데코레이터랑 구현 방식이 비슷한데 목적이 다르다. 데코레이터는 기능을 더하는 것이 목적이고 프록시는 접근을 제한하는 것이 목적이다.
	...
	namespace Behavioural
	{
		...
		{
			// 송신자와 수신자를 디커플링 시킨다.
			// 도움 메뉴에서 도움 메세지를 띄울 때에 가장 자세한 사항을 띄워야 하는데, 
		...
		namespace Command
		{
			//요청을 캡슐화 하는 것, 이를 통해 동작을 취소하거나 매개변수를 쉽게 바꿀 수 있는 요청이 가능하도록 하는 것.
		...
		namespace Visitor
		{
			...
			// 순회하면서 무언가 상태를 쌓아 둬야 할 때에도 쓰면 좋다.
